...
function CSS3MultiColumn() {
  ...
  var cssCache = {};
  var splitableTags = [
    'P', 'DIV', 'SPAN', 'BLOCKQUOTE', 'ADDRESS', 'PRE', 'A', 'EM', 'I',
    'STRONG', 'B', 'CITE', 'OL', 'UL', 'LI', 'ARTICLE', 'SECTION', 'ASIDE',
  ...
  var debug = ut.debug;
  if (document.location.search.match('mode=debug'))
    var isDebug = true;
  else
    var isDebug = false;
  ...
  // INITIALIZATION
  ut.XBrowserAddEventHandler(window, 'load', function() {
    documentReady = true;  processElements();
  });
  loadStylesheets();
  ...
  function processElement(affectedElement, column_count, column_width, column_gap, column_rule) {
    ...
    for (var i = 1; i < column_count && elem && j < (column_count + 5); i++) {
      ...
      if (isDebug) {
        bestSplitPoint.style.border = "1px solid #00FF00";
      }
    ...
    for (var i = 1; i < column_count && elem; i++) {
      ...
      if (bestSplitPoint && isElementSplitable(bestSplitPoint) && elem.id != bestSplitPoint.id) {
        ...
        if (isDebug) {
          secondSplitPoint.style.border = "1px dotted #00F";
        }
      } else {
      ...
      if (isDebug) {
        splitE.style.border = "1px solid #F00";
      }
    ...
    if (navigator.userAgent.toLowerCase().indexOf('safari') + 1) {
      wrapper.innerHTML += ' ';
    } // forces redraw in safari and fixes bug above.
  ...
  function cssQuery(selector, from) {
    ...
    for (var i in selectors) {
      ...
      while (j < selector.length) {
        ...
        if (useCache) {
          cssCache[cacheSelector] = filtered;
        }
      }
    ...
    switch (token) {
      ...
      case ">": // child
        for (i in from) {
          ...
          for (var j = 0; j < subset.length; j++)
            if (compareTagName(subset[j], filter, namespace)) {
              push(filtered, subset[j]);
          }
        }
        break;
      case "+": // adjacent (direct)
        for (i in from) {
          ...
          if (adjacent && compareTagName(adjacent, filter, namespace)) {
      ...
      case ".": // class
        ...
        for (i in from)
          if (filter.test(from[i].className)) {
            push(filtered, from[i]);
        }
        break;
      case "#": // id
        for (i in from)
  ...
  if (target.addEventListener) {
    ...
    if (originalHandler) {
      target["on" + eventName] = function(e) {
        originalHandler(e);eval(handlerName)(e);
      };
    } else {
